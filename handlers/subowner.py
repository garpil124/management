...
try:
    subowners_col = getattr(db_mongo, "subowners_col")
except Exception:
    subowners_col = getattr(db_mongo, "partners_col", None)

# ============================================================
# Helper: detect async / sync collection
# ============================================================

def _is_async_col(col):
    return asyncio.iscoroutinefunction(getattr(col, "find_one", None))


async def _insert_doc(col, doc: dict):
    if _is_async_col(col) and asyncio.iscoroutinefunction(col.insert_one):
...
async def _find_one(col, *args, **kwargs):
    if _is_async_col(col):
        return await col.find_one(*args, **kwargs)
    else:
        return col.find_one(*args, **kwargs)


async def _find_many(col, *args, **kwargs):
    if _is_async_col(col):
        cursor = col.find(*args, **kwargs)
        return [d async for d in cursor]
    else:
        return list(col.find(*args, **kwargs))


async def _update_one(col, filter_q, update_q, upsert=False):
    if _is_async_col(col) and asyncio.iscoroutinefunction(col.update_one):
        return await col.update_one(filter_q, update_q, upsert=upsert)
    else:
        return col.update_one(filter_q, update_q, upsert=upsert)


async def _delete_one(col, filter_q):
    if _is_async_col(col) and asyncio.iscoroutinefunction(col.delete_one):
        return await col.delete_one(filter_q)
    else:
        return col.delete_one(filter_q)


# ============================================================
# In-memory pending flows per user
# ============================================================

PENDING = {}  # {user_id: {"flow":..., "step":..., "data":...}}

DEFAULT_PREMIUM_DAYS = 30
MAX_SUBOWNERS = 5  # optional guard
...
def _owner_keyboard():
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("➕ Add Subowner", callback_data="sub:owner:add")],
        [InlineKeyboardButton("📋 List Subowners", callback_data="sub:owner:list")],
    ])


def _subowner_kb():
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("⚙️ Settings Store", callback_data="sub:settings")],
        [InlineKeyboardButton("📄 Info Akun", callback_data="sub:info")],
    ])
...
def register_subowner(app: Client):
    """Register subowner-related handlers. Call this with the pyrogram Client instance."""
    ...
    async def cmd_addsub_owner(client: Client, message: Message):
        ...
        try:
            ...
            try:
                ...
                return await message.reply(
                    "LOG_GROUP_ID dan MAIN_GROUP_ID harus berupa angka (contoh -100123456789)"
                )
            ...
            try:
                ...
                if isinstance(current, list) and len(current) >= MAX_SUBOWNERS:
                    return await message.reply(
                        f"⚠️ Maksimal subowner = {MAX_SUBOWNERS}. Hapus dulu jika ingin tambah."
                    )
            except Exception:
                pass
    ...
    async def owner_sub_cb(client: Client, callback: CallbackQuery):
        ...
        if action == "add":
            ...
            try:
                ...
                return await callback.answer("Gagal memuat list", show_alert=True)

        await callback.answer()

    # ================= Subowner command via user =================

    @app.on_message(filters.command("become") & filters.private)
    async def cmd_become_subowner(client: Client, message: Message):
        await message.reply(
            "🔒 Untuk menjadi subowner, pemilik harus menambahkan via /addsub.\nHubungi owner."
        )
    ...
    async def sub_settings_flow_cb(client: Client, callback: CallbackQuery):
        ...
        if key == "name":
            ...
            await callback.message.edit_text("💳 Kirim instruksi pembayaran baru:")
        elif key == "log":
        ...
        try:
            if state.get("flow") == "settings":
                ...
                if key == "name":
                    ...
                    return await message.reply("✅ Nama store diperbarui.")

                if key == "payment":
                    await _update_one(subowners_col, {"_id": doc.get("_id")}, {"$set": {"payment_info": value}})
                    return await message.reply("✅ Instruksi pembayaran diperbarui.")

                if key == "log":
                    try:
                        gid = int(value)
                    except Exception:
                        return await message.reply("⚠️ Format ID grup salah. Harus angka.")

                    await _update_one(subowners_col, {"_id": doc.get("_id")}, {"$set": {"log_group": gid}})
                    return await message.reply("✅ Grup log diperbarui.")
